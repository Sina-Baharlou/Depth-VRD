...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, multiple_preds=False):
        ...
        self.result_dict[self.mode + '_recall_per_rel']= {20: [], 50: [], 100: []}
    ...
    def print_stats(self, epoch_num=None, writer=None):
        # print('======================' + self.mode + '============================')
        ...
        for k, vs in self.result_dict[self.mode + '_recall_per_rel'].items():
            ...
            filenames =[]
            local_fname = None
            hits_per_rel_per_batch = dict()
            gt_cnt_per_rel_per_batch = dict()
            ...
            # print('Micro from Macro R@%i: %f' % (k, micro_from_macro))

            # print('R@%i - rel %i: %f' % (k, rel, np.mean(prob)))
            # if writer is not None:
            #     writer.add_scalar('data/R@%i/%i' % (k, rel), np.mean(v), epoch_num)
