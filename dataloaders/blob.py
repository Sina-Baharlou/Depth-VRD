...
class Blob(object):
    def __init__(self, mode='det', is_train=False, num_gpus=1, primary_gpu=0, batch_size_per_gpu=3, use_depth=False):
        """
    ...
    def append(self, d):
        ...
        # -- Add depth images to the blob
        if self.use_depth:
            self.depth_imgs.append(d['depth_img'])
    ...
    def reduce(self):
        ...
        # -- Perform reduction on depth data
        if self.use_depth:
            self.depth_imgs = Variable(torch.stack(self.depth_imgs, 0), volatile=self.volatile)
    ...
    def scatter(self):
        ...
        # -- Load depth data into the gpu
        if self.use_depth:
            self.depth_imgs = self._scatter(self.depth_imgs, [self.batch_size_per_gpu] * self.num_gpus)
    ...
    def __getitem__(self, index):
        ...
        if index == 0 and self.num_gpus == 1:
            image_offset = 0
            if self.is_train:
                # -- Add depth data in return function
                return (self.imgs, self.im_sizes[0], image_offset,
                        self.gt_boxes, self.gt_classes, rels, proposals, self.train_anchor_inds, False,
                        self.depth_imgs if self.use_depth else None)

            # -- Add depth data in return function
            return (self.imgs, self.im_sizes[0], image_offset, self.gt_boxes, self.gt_classes, rels, proposals, None,
                    False, self.depth_imgs if self.use_depth else None)
        ...
        if self.is_train:
            ...
            return (
                ...
                False, self.depth_imgs[index] if self.use_depth else None)

        # -- Add depth data in return function
        return (self.imgs[index], self.im_sizes[index], image_offset,
                self.gt_boxes[index], self.gt_classes[index], rels_i, None, None,
                False, self.depth_imgs[index] if self.use_depth else None)
