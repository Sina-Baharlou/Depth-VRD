...
def random_crop(img, boxes, box_scale, round_boxes=True, max_crop_fraction=0.1):
    ...
    new_boxes = box_scale / max(img_cropped.size) * np.column_stack(
        (boxes_scaled[:, 0] - crop_left, boxes_scaled[:, 1] - crop_top, boxes_scaled[:, 2] - crop_left,
         boxes_scaled[:, 3] - crop_top))
...
class RepeatTensor(object):
    """
    Repeat the tensor in the channels' axis (convert a depth image into three channels)
    """

    def __call__(self, img):
        """
        :param img: an image tensor with a single channel
        :return: an image tensor with three channels
        """
