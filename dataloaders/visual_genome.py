...
from config import VG_IMAGES, VG_DEPTH_IMAGES, IM_DATA_FN, VG_SGG_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN
...
class VG(Dataset):
    ...
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
                 ...
                 use_proposals=False, use_depth=False, three_channels_depth=False):
        """
        ...
        # -- Signature enhanced
        # -- Load RGB images' filenames --
        self.filenames = load_image_filenames(image_file,VG_IMAGES, is_depth=False)
        ...
        # -- Initialize depth related items
        self.use_depth = use_depth
        self.three_channels_depth = three_channels_depth

        if self.use_depth:
            # -- Load depth images' filenames --
            self.depth_filenames = load_image_filenames(image_file, VG_DEPTH_IMAGES, is_depth=True)
            self.depth_filenames = [self.depth_filenames[i] for i in np.where(self.split_mask)[0]]

            # -- define a transformation pipeline for depth images
            depth_tform = [
            ...
            # -- Convert depth image into three channels
    ...
    def __getitem__(self, index):
        ...
        # -- Determine RGB image final size --
        ...
        # -- Process depth images if the flag is set --
        if self.use_depth:
            # -- Load depth image (as np.int32)
            depth_filename = self.depth_filenames[index]
            depth_img_unpadded = Image.open(depth_filename)

            # -- flip the image
            if flipped:
                depth_img_unpadded = depth_img_unpadded.transpose(Image.FLIP_LEFT_RIGHT)

            # -- Apply transformations to the image
            transformed_depth = self.depth_transform_pipeline(depth_img_unpadded)

            # -- Create a depth entry containing the related information
            depth_entry = {
                'depth_img': transformed_depth,
                'depth_fn': depth_filename
            }
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def assertion_checks(entry, use_depth=False, three_channels_depth=False):
    ...
    # -- insert assertions checks for depth image
    if use_depth:
        depth_im_size = tuple(entry['depth_img'].size())
        if len(depth_im_size) != 3:
            raise ValueError("Depth img must be dim-3")

        c_d, h_d, w_d = entry['depth_img'].size()
        depth_channels = 3 if three_channels_depth else 1
        if c_d != depth_channels:
            raise ValueError("Incorrect number of depth channels.")
...
def load_image_filenames(image_file, image_dir=VG_IMAGES, is_depth=False):
    """
    ...
    # -- enhanced the corrupted images to support both rgb and depth images
    corrupted_ids = [1592, 1722, 4616, 4617]
    # -- Specify the image format (`png` for depth images and `jpg` for RGB ones)
    fmt = "png" if is_depth else "jpg"
    fns = []
    for i, img in enumerate(im_data):
        # -- directly check img['image_id'] instead of the basename
        if img['image_id'] in corrupted_ids:
            continue
        # -- Add file extension here
        basename = '{}.{}'.format(img['image_id'], fmt)
...
def vg_collate(data, num_gpus=3, is_train=False, mode='det', use_depth=False):
    assert mode in ('det', 'rel')
    # -- Pass depth flag
    blob = Blob(mode=mode, is_train=is_train, num_gpus=num_gpus,
                batch_size_per_gpu=len(data) // num_gpus, use_depth=use_depth)
    for d in data:
        blob.append(d)
    blob.reduce()
    return blob
...
class VGDataLoader(torch.utils.data.DataLoader):
    ...
    def splits(cls, train_data, val_data, batch_size=3, num_workers=1, num_gpus=3, mode='det',
        ...
        train_load = cls(
            ...
            # -- pass depth flag
            collate_fn=lambda x: vg_collate(x, mode=mode, num_gpus=num_gpus, is_train=True, use_depth=use_depth),
            drop_last=True,
            # pin_memory=True,
            **kwargs,
        )
        val_load = cls(
            ...
            num_workers=num_workers,
            # -- pass depth flag
            collate_fn=lambda x: vg_collate(x, mode=mode, num_gpus=num_gpus, is_train=False, use_depth=use_depth),
            drop_last=True,
            # pin_memory=True,
            **kwargs,
        )
